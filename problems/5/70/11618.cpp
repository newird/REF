...
public:
	...
	friend void pop();
	friend void tail_insert(long long k);
	friend void head_insert(long long k);
};

stack1* head = nullptr;
stack1* tail = nullptr;

void tail_insert(long long k)
{
...
int main()
{
	...
	for (int i = 1; i <= n; i++)
	{
		...
		if (length < m && arr[i]>0)
		{
			if (length == 0)
				head_insert(arr[i]);
			else if (arr[i] >= tail->data)
				tail_insert(arr[i]);
			else if (arr[i] <= head->data)
				head_insert(arr[i]);
			else {
				if (tail->data - arr[i] <= arr[i] - head->data)
					tail_insert(arr[i]);
				else
					head_insert(arr[i]);
			}
