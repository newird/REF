...
void down(int arr[],int way,int am){
	...
	if (2 * way + 1 < am) { if (arr[way] > arr[2 * way + 1]) {  int v=arr[way]; arr[way] = arr[2 * way + 1]; way = 2 * way + 1; arr[way] = v; } }
}

int main(){
	int n,m,d,num,am=0,max=0,sum=0;
	cin>>n>>m>>d;
	int arr[m];
	for(int i=1;i<n+1;i++){
		cin>>num;
