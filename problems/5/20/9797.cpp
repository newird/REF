...
int main()
{
    ...
    for (int i = 0; i != n; i++)
    {
        if (happy[i] > 0)
        {
            ...
            {
                temp = heap.sum;
                if (temp - (d * (last_index + 1)) > heap.try_new(happy[i]) - (d * (i + 1)))
                {
                    heap.Remove(temp);
                    heap.Insert(happy[i]);
                    long long pre = temp - (d * (last_index + 1));
                    long long current = heap.try_new(happy[i]) - (d * (i + 1));
                    max_sum = (pre > current) ? pre : current;
                }
            }
            else
            {
                temp = heap.sum;
                heap.Insert(happy[i]);
                max_sum = ((temp - d * (last_index + 1)) > (heap.sum - d * (i + 1))) ? (temp - d * (last_index + 1)) : (heap.sum - d * (i + 1));
                last_index = i;
            }
        }
        else
