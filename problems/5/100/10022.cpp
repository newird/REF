...
struct MinHeap 
{
    ...
    void push(long long int value)  //pemasukan minheap
    {
        arr[size++] = value;
        int index = size - 1;
        while (index > 0)
    ...
    void pop() //ya pop biasa
    {
        ...
        arr[0] = arr[--size];
        int index = 0;
        while (true) // pop, lalu rearrange sampe selesai aja pokoknya
        {
            int left = 2 * index + 1;
            int right = 2 * index + 2;
            int smallest = index;
...
int main() 
{
    ...
    for (long long int i = 0; i < n; i++)
    {
        if (arr[i] > 0) 
        {
            ...
            {
                sum = sum - a.top() + arr[i]; // klo lebih gehde tukeran, yang tadi di minus yang tadi diplus
                a.pop();
                a.push(arr[i]);
            }
