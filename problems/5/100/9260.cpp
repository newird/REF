...
int main() {
	...
	cin >> n >> m >> d;
	long long a[200000]; long long b[200000];
	...
	if (m == 0) { cout << 0; }
		...
		for (long long i = 0; i < n; i++) {
			...
			if (size == m - 1 && flag == 1) { for (long long j = m - 2; j >= 0; j--) { Adjust(b, j, m - 1); }flag = 0; }
			else if (m != 1 && size == m - 1 && i >= 1 && a[i - 1] > b[0]) {
				t -= b[0]; t += a[i - 1]; b[0] = a[i - 1]; Adjust(b, 0, m - 1);
			}
